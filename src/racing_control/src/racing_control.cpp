...
RacingControlNode::RacingControlNode(const std::string& node_name,const rclcpp::NodeOptions& options)
  ...
  // own -------------------------------------------------------------------------------
  patrol_command_ = true;
  parking_begin_ = false;

  patrol_command_subscriber_ = this->create_subscription<std_msgs::msg::Bool>(
      "/patrol_command",
      10,
      std::bind(&RacingControlNode::patrol_command_callback,
      this,
      std::placeholders::_1));

  traffic_light_subscriber_ = this->create_subscription<ai_msgs::msg::Point>(
      "/traffic_light_detection",
      10,
      std::bind(&RacingControlNode::traffic_light_callback,
      this,
      std::placeholders::_1));

  parking_begin_subscriber_ = this->create_subscription<std_msgs::msg::Bool>(
      "/parking_begin",
      10,
      std::bind(&RacingControlNode::parking_begin_callback,
      this,
      std::placeholders::_1));

  patrol_result_publisher_ = this->create_publisher<std_msgs::msg::Bool>("patrol_result", 5);
  // own -------------------------------------------------------------------------------
...
// own -------------------------------------------------------------------------------
void RacingControlNode::patrol_command_callback(const std_msgs::msg::Bool::SharedPtr bool_msg)
{
  patrol_command_ = bool_msg->data;
}

void RacingControlNode::parking_begin_callback(const std_msgs::msg::Bool::SharedPtr bool_msg)
{
  parking_begin_ = bool_msg->data;
}

void RacingControlNode::traffic_light_callback(const ai_msgs::msg::Point::SharedPtr point_msg)
{
  // TODO:
}
// own -------------------------------------------------------------------------------
      ...
      if (patrol_command_)
      {
        if (parking_begin_ == false)
        {
          ...
          else 
          {
            ...
            {
              if (target.type == "construction_cone") // 检查检测物的类别
              {
                /* 得到检测框的最大 y 坐标 */
